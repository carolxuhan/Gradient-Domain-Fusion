close all
clear
imga = im2double(imread('s3.jpg'));
imgb = im2double(imread('s4.jpg')); % size(imga) = size(imgb)
...
imga(:,v+1:end,:)=0;imgb(:,1:v,:)=0;
imwrite(imga+imgb,'raw3.jpg');
imwrite(imgo1,'iwant.jpg');